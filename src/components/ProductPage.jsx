  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-3 sm:px-4 md:px-6 py-5 sm:py-8 overflow-x-hidden min-w-0">
      <div className="flex items-center gap-2 sm:gap-3 mb-5 sm:mb-6">
        <button
          onClick={handleBack}
          className="flex items-center gap-2 font-mono text-xs text-[var(--ink)] bg-white border border-[var(--line)] px-3.5 sm:px-4 py-2 rounded-full hover:bg-[var(--mustard)] hover:text-black font-semibold transition-all shadow-sm group active:scale-95 cursor-pointer"
        >
        ...
        <span className="font-mono text-xs opacity-60 truncate max-w-[120px] sm:max-w-xs">{product.name}</span>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-10">
        ...
        <div>
              ...
              {availableSizes.map((s) => {
                ...
                return (
                  <button
                    ...
                    className={`min-w-0 sm:min-w-[72px] px-2.5 sm:px-3.5 py-2 sm:py-2.5 rounded-lg border text-center transition-all relative cursor-pointer ${
                      isSelected
                  ...
                  >
                    <div className="flex items-center justify-center gap-1 font-bold text-xs sm:text-sm">
                      <span>{s}</span>
                      {isSelected ? (
                        <span className="inline-flex items-center justify-center w-3.5 sm:w-4 h-3.5 sm:h-4 rounded-full bg-[var(--mustard)] text-[var(--ink)] text-[9px] sm:text-[10px] font-extrabold ml-0.5">
                          ✓
                        </span>
                      ) : (
                        <span className="inline-block w-3 sm:w-3.5 h-3 sm:h-3.5 rounded-full border border-gray-300 ml-0.5 opacity-40"></span>
                      )}
                    </div>
                    <span
                      className={`text-[8.5px] sm:text-[9.5px] font-mono block mt-0.5 ${
                        isSelected
          ...
          <div className="mt-7 flex flex-col sm:flex-row gap-3">
            ...
            >
              <Heart size={15} fill={wishlist && wishlist.includes(product.id) ? "var(--oxblood)" : "none"} color="var(--oxblood)" />
              <span>Save</span>
            </button>
          </div>
      ...
      <section className="mt-12 sm:mt-16 relative w-full overflow-x-hidden min-w-0">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-5 min-w-0 w-full">
          {related.map((p, i) => (
            <ProductCard
              key={p.id}
              p={p}
